...
function formatRepliesMessage(data: unknown): string {
  ...
  const lines = items.slice(0, 10).map((item, i) => {
    ...
    const bodySource: string =
      ...
      typeof item.bodyHtml === "string" ? item.bodyHtml.replace(/<[^>]+>/g, " ") : "";
    const preview = bodySource.replace(/\s+/g, " ").trim().slice(0, 160);
    ...
    const previewLine = preview ? `\n   ${preview}${bodySource.length > 160 ? "…" : ""}` : "";
    return `${i + 1}. **${meta}**${subjectLine}${previewLine}`;
  });
...
function buildResponse(state: AgentGraphStateType): string {
  ...
  if (toolResult !== undefined) {
    ...
    if (toolName === "parse_csv_file") {
      ...
      const hasMoreRows = validRows > previewLines.length;
      ...
      const msg = [
        ...
        "",
        hasMoreRows
          ? "Reply **yes** to save, **review all** to see the full recipient list, or **discard** to cancel."
...
async function maybeEnhanceWithOpenAI(
  ...
  // NOTE: get_campaign_stats / list_replies used to get an extra OpenAI pass here
  // purely to reword the already-correct deterministic message into more natural
  // prose ("enhanceResponse"). Removed — it added a full LLM round-trip to two of
  // the most frequently used intents for a cosmetic-only improvement; the
  // deterministic baseResponse is already accurate and complete.
...
export async function finalResponseNode(
  ...
  const withResumeSuggestion = maybeAppendResumeSuggestion(state, enhanced);
  const response = maybeAppendUnreadReplyNotice(state, withResumeSuggestion);
...
/** Appends the proactive unread-reply nudge (set by executeTool.node's inbox peek) to the response, if present. */
function maybeAppendUnreadReplyNotice(state: AgentGraphStateType, response: string): string {
  const notice = state.unreadReplyNotice;
