...
// ── 2. get_campaign_stats / list_replies — deterministic message, no LLM pass ─
//
// These intents used to get an extra OpenAI "enhanceResponse" pass purely to
// reword the already-correct deterministic message into more natural prose.
// That pass was removed (latency: it added a full LLM round-trip to two of
// the most frequently used intents for a cosmetic-only improvement). These
// tests lock in that removal — the deterministic message is used as-is and
// enhanceResponse is never invoked for these intents.

describe("get_campaign_stats / list_replies — no OpenAI enhancement pass", () => {

  it("get_campaign_stats: uses the deterministic message and never calls enhanceResponse", async () => {
    ...
    // No LLM round-trip for this intent anymore
    expect(mockEnhanceResponse).not.toHaveBeenCalled();
  });

  it("list_replies: uses the deterministic message and never calls enhanceResponse", async () => {
    const repliesData = {
      items:       [{ id: "1", fromEmail: "alice@example.com" }],
      total:       85,
      hasNextPage: true,
    };
...
describe("list_replies — formats actual reply items", () => {
  ...
  // Regression test: the real MCP tool wraps its result in a
  // { success: true, data: {...} } envelope (mcpToolCaller normalizes MCP
  // content into { data: <raw tool JSON> }), so toolResult.data is actually
  // { success: true, data: { items, total } } — not the flat shape used by
  // the fixture above. formatRepliesMessage() must unwrap this envelope or
  // it silently reports "no replies" even when replies exist. It must also
  // read the real Reply field names (fromEmail/subject/bodyText — not the
  // sender/preview/body names the fixture above used), or the subject and
  // body silently drop out of the response even when items ARE found.
  it("shows fromEmail, subject, and bodyText when toolResult.data uses the real MCP Reply shape", async () => {
    mockEnhanceResponse.mockRejectedValue(new Error("skip"));
    const state = makeState({
      intent:      "list_replies",
      toolName:    "list_replies",
            ...
            items: [
              {
                ...
                subject:    "Re: Revisiting Our Previous Conversation",
    ...
    expect(parsed.message).toContain("carol@example.com");
    expect(parsed.message).toContain("Re: Revisiting Our Previous Conversation");
    expect(parsed.message).toContain("Sounds great, let's talk.");
    expect(parsed.message).not.toMatch(/no replies/i);
  });
});
