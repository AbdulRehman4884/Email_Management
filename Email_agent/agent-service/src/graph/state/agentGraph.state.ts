...
export const AgentGraphState = Annotation.Root({
  ...
  /**
   * Short note appended to this turn's response when new unread inbox
   * replies were found by the post-tool-execution peek in executeTool.node.
   * Transient — recomputed every turn, never persisted to session memory.
   */
  unreadReplyNotice: Annotation<string | undefined>({
    reducer: replace,
    default: () => undefined,
  }),

  /**
   * Unread-reply count last surfaced to the user via unreadReplyNotice.
   * Persisted across turns so the agent only re-mentions replies once the
   * count has grown past what it already told the user about.
   */
  lastNotifiedUnreadReplyCount: Annotation<number | undefined>({
    reducer: replace,
    default: () => undefined,
  }),

  /** Counts turns since the last unread-reply peek, so the check only runs periodically, not on every turn. */
  turnsSinceLastReplyCheck: Annotation<number | undefined>({
    reducer: replace,
    default: () => undefined,
  }),
