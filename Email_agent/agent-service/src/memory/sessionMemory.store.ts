...
export interface SessionSnapshot {
  ...
  /** Unread-reply count last mentioned to the user, so the proactive nudge only fires again once it grows. */
  readonly lastNotifiedUnreadReplyCount?: number;

  /** Turns since the last unread-reply peek (see turnsSinceLastReplyCheck in agentGraph.state.ts). */
  readonly turnsSinceLastReplyCheck?: number;
