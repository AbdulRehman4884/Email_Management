...
export function CreateCampaign() {
  ...
  const handleSkipUpload = () => navigate(`/campaigns/${createdCampaignId}`);
  ...
  return (
    <div className={`mx-auto space-y-6 ${currentStep === 2 ? 'max-w-5xl' : 'max-w-3xl'}`}>
              ...
              {scheduleEnabled && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Input
                    ...
                    name="scheduledTime"
                    type="time"
                    value={formData.scheduledAt?.split('T')[1] || '09:00'}
                    onChange={(e) => {
          ...
          <CardContent className="py-6 px-8">
            <div className="space-y-5">
              ...
              {!useCustomHtml && templateId === 'simple' && (
                <>
                  <Input label="Heading" name="heading" value={templateData.heading || ''} onChange={(e) => handleTemplateDataChange('heading', e.target.value)} placeholder="Your email heading" error={formErrors.heading} />
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className="block text-sm font-medium text-gray-700">Body</label>
                      <div className="flex gap-1">
                        {['{{firstName}}', '{{lastName}}', '{{email}}', '{{company}}'].map(token => (
                          <button key={token} type="button" onClick={() => insertToken(token)} className="px-2 py-0.5 text-xs bg-gray-100 text-gray-600 rounded hover:bg-gray-200 transition-colors font-mono">
                            {token}
                          </button>
                        ))}
                      </div>
                    </div>
                    <TextArea name="body" value={templateData.body || ''} onChange={(e) => handleTemplateDataChange('body', e.target.value)} rows={5} placeholder="Hi {{firstName}}, I wanted to reach out..." error={formErrors.body} />
              ...
              {!useCustomHtml && templateId === 'announcement' && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Input label="Link URL (optional)" type="url" value={templateData.linkUrl || ''} onChange={(e) => handleTemplateDataChange('linkUrl', e.target.value)} />
                    <Input label="Link text (optional)" value={templateData.linkText || ''} onChange={(e) => handleTemplateDataChange('linkText', e.target.value)} />
                  </div>
                </>
              )}
              {!useCustomHtml && templateId === 'newsletter' && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Input label="Main link URL (optional)" type="url" value={templateData.mainLinkUrl || ''} onChange={(e) => handleTemplateDataChange('mainLinkUrl', e.target.value)} />
                    <Input label="Main link text (optional)" value={templateData.mainLinkText || ''} onChange={(e) => handleTemplateDataChange('mainLinkText', e.target.value)} />
                  </div>
                  <Input label="Footer (optional)" value={templateData.footer || ''} onChange={(e) => handleTemplateDataChange('footer', e.target.value)} />
                </>
              )}
              {useCustomHtml && (
                <TextArea
      ...
      {currentStep === 3 && (
        <Card>
          <CardContent className="py-6">
            <h2 className="text-base font-semibold text-gray-900 mb-4">Upload recipients</h2>
            {!uploadResult ? (
              <div className="space-y-4">
                ...
                >
                  ...
                  {uploadedFile ? (
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setUploadedFile(null); }} className="p-1 hover:bg-gray-200 rounded transition-colors">
                    ...
                    <>
                      ...
                      <button type="button" className="mt-3 px-4 py-1.5 text-sm border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                        Choose file
                      </button>
                    </>
                  )}
                ...
                <p className="text-xs text-gray-500">
                  Required column: <code className="text-gray-700 font-medium">email</code>. Optional: <code className="text-gray-700 font-medium">name</code>, <code className="text-gray-700 font-medium">firstName</code>, <code className="text-gray-700 font-medium">lastName</code>
                </p>
                {uploadedFile && (
      ...
      <div className="flex justify-between items-center pt-2">
        {currentStep > 1 ? (
          <button onClick={handleBack} className="flex items-center text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors">
            <ArrowLeft className="w-4 h-4 mr-1.5" /> Back
          </button>
        ) : (
          <button onClick={() => navigate('/campaigns')} className="flex items-center text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors">
        ...
        <div className="flex items-center gap-3">
          {currentStep === 3 && !uploadResult && (
            <Button variant="secondary" onClick={handleSkipUpload}>Save as draft</Button>
          )}
          {currentStep < 3 && (
